...
# --- PHẦN 1: TẠO SERVER WEB ĐỂ GIỮ BOT LUÔN THỨC (KEEP ALIVE) ---
app = Flask('')

@app.route('/')
def home():
    return "Bot Valorant đang chạy ổn định!"

def run_flask():
    # Render yêu cầu chạy trên port được cấp phát (mặc định 8080)
...
# --- PHẦN 2: LOGIC XỬ LÝ DỮ LIỆU VALORANT ---
load_dotenv()
...
def get_val_details(name, tag):
    ...
    try:
        ...
        if r_mmr.status_code == 200 and r_hist.status_code == 200:
            ...
            if not m_data: 
                return {"status": "error", "msg": "Không có dữ liệu", "rank_icon": default_icon, "name": name, "tag": tag}
            ...
            for m in h_data:
                ...
                # FIX LỖI DẤU TRÙNG LẶP: Dùng abs() để lấy giá trị tuyệt đối
                if change > 0:
                    mark, val = "+ ", change
                elif change < 0:
                    mark, val = "- ", abs(change)
        ...
        return {"status": "error", "msg": f"API Error ({r_mmr.status_code})", "rank_icon": default_icon, "name": name, "tag": tag}
    except:
        return {"status": "error", "msg": "Lỗi kết nối API", "rank_icon": default_icon, "name": name, "tag": tag}

@bot.command()
async def rank(ctx):
    await ctx.send("🔍 **Đang quét và sắp xếp danh sách theo MMR giảm dần...**")
    
    results = []
    for acc in MY_ACCOUNTS:
        res = get_val_details(acc['name'], acc['tag'])
        results.append(res)

    # Lọc các acc thành công và sắp xếp giảm dần theo Elo (MMR)
    ...
    embed = discord.Embed(
        ...
        description="Danh sách tài khoản sắp xếp theo **MMR (Elo)** cao nhất xuống thấp nhất.",
        color=0xfa4454
    )
    ...
    for res in success_data:
        ...
        # Thumbnail lấy hình rank của người đứng đầu danh sách
...
@bot.event
async def on_ready():
    print(f'✅ Bot online! Gõ ?vrank để kiểm tra.')
